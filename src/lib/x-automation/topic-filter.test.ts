...
import { parseTrendTraffic } from "@/lib/x-automation/social-trends";
...
test("話題カテゴリーは検索トレンド候補だけを拾う", () => {
  assert.equal(candidateMatchesTopic(candidate({ category: "trend", themeKey: "trend:半導体" }), "social_trends"), true);
  assert.equal(candidateMatchesTopic(candidate({}), "social_trends"), false);
  assert.equal(parseTrendTraffic("10K+"), 10_000);
  assert.equal(parseTrendTraffic("5万+"), 50_000);
});
...
test("異常候補がない時も市場値と公式指標から概況候補を作る", () => {
  const input = {
    ...
    news: [], newsMode: "official", investorSignals: [], socialTrends: [], socialMode: "unavailable",
